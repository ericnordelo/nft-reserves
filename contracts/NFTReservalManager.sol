...
contract NFTReservalManager {
    ...
    event EReservalCanceled(address owner, uint256 reservalID);
    ...
    event EOfferCanceled(address buyer, uint256 offerID);

    event EOfferAccepted(uint256 offerID);

    event EAssigned(bool assigned, uint256 reservalID, uint256 offerID);

    event ECollateralDeposit(uint256 offerID, address token, uint256 amount, bool isDeposit);

    event EReservePayed(uint256 offerID);
    ...
    function registerValidToken(address[] memory _tokens) external {
        tokens = _tokens;
        TOKEN_CNT = _tokens.length;
        for (uint256 i = 0; i < TOKEN_CNT; i++) {
    ...
    function updateReserval(
        ...
        NFTReserval memory _reserval = NFTReserval(
            msg.sender,
            nft,
            nftID,
            expiry,
    ...
    function cancelReserval(uint256 reservalID) external {
        NFTReserval storage reserval = _getReserval(reservalID);
        require(reserval.owner != address(0), "No such reserval exists");
        require(reserval.owner == msg.sender, "Not owner of reserval");
    ...
    function updateOffer(
        ...
        emit EOfferUpdated(_offer.buyer, reservalID, nft, nftID, expiry, token, price, pct, offerID);
    }
    ...
    function acceptOffer(uint256 offerID) external {
        ...
        for (uint256 i = 0; i < offer.colTokens.length; i++) {
    ...
    function assignOffer(uint256 offerID) external {
        Offer storage offer = _getOffer(offerID);
        require(offer.buyer != address(0), "No such offer exists");
    ...
    function payReserve(uint256 offerID) external {
        Offer storage offer = _getOffer(offerID);
        require(offer.buyer != address(0), "No such offer exists");
        require(offer.accepted, "Not accepted offer");
    ...
    function withdrawCollateral(
        ...
        require(
            (offer.colTokens.length > id) && (offer.colAmounts[id] >= amount),
            "Not enough amount deposited"
        );
    ...
    function _putOut(
        ...
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(SELECTOR, account, amount));
    ...
    function _tokenToUSD(address token) private view returns (uint256) {
